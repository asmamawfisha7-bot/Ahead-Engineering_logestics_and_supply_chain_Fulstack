...
<form action="UpdateStockInAc.php" method="post" name="update" onsubmit="return checkvalidation()">
<fieldset id=""><legend align="center"> <font face='calibri' size='5px' color="brown">Purchased, Returned and Transfer In Accesseries Modification</font></legend> <br><br>
<?php 
 include('Connection.php');

 include('Connection.php');

 if (isset($_POST['UpdateStockInAc'])) {
	 $date=$_POST['date'];
...
$select_query = "SELECT  * FROM balancep where generalname='Accesseries' order by producttype"; 
  ...
  while($row = mysqli_fetch_row($select_result)) {
				 echo '<tr width:200px;><form action="UpdateStockInAc.php" method="post" >';
				 ...
				 echo "<td><input name='UpdateStockInAc' type=submit value=update Onclick='return ConfirmUpdate();'> </td></form></tr>";
			   }
			 echo "</table>"; 
