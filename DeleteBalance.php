...
<form action="DeleteBalance.php" method="post" name="delete" onsubmit="return checkvalidation()">
<fieldset id=""><legend align="center"> <font face='calibri' size='5px' color="brown">Deleting Balance Page</font></legend> <br><br>
<?php 
include ('Conn.php');
     $select_query = "select * from balancep order by date DESC"; 
    ...
    echo "<table id='qq' style='width:1000px;border:1px solid #336699;border-radius:10px;' align='center'><font color=white>";
     ...
     while($row = mysqli_fetch_row($select_result)) {
					echo '<tr><form action="DeleteBalance.php" method="post">';
					...
					echo "<td><input style='width:110px;' value='".$row[6]."' name=deliveredquantity pattern='[0-9]{1,10}' title='please fill Withdraw Number only' > </td>";
					echo "<td><input style='width:110px;' value='".$row[7]."' name=balancequantity readonly pattern='[0-9]{1,10}' title='please fill Withdraw Number only' > </td>";
					...
					echo "<td><input type=submit name='DeleteBalance' value='Delete' Onclick='return ConfirmDelete();'> </td></form></tr>";
	              }
