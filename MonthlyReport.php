...
<form action="MonthlyReport.php" method="post" name="delete" onsubmit="return checkvalidation()">
<fieldset id=""><legend align="center"> <font face='calibri' size='5px' color="brown">View All Profile Monthly Reports </font></legend> <br><br>
 <center>&nbsp;&nbsp;&nbsp;
<?php
include('Conn.php');
$result = mysqli_query($data, "SELECT date,productid,generalname,productname,producttype,unit,deliveredquantity,balancequantity,
store FROM balancep where deliveredquantity>1");
$d=mysqli_num_rows($result);
$result2 = mysqli_query($data, "SELECT date,productid,generalname,productname,producttype,unit,deliveredquantity,balancequantity,
store FROM balancep where deliveredquantity>1");
$f=mysqli_num_rows($result2);
$result3 = mysqli_query($data, "SELECT date,productid,generalname,productname,producttype,unit,deliveredquantity,balancequantity,
store FROM balancep where deliveredquantity>1");
...
echo"<td><font color='black'>Delivered &nbsp;Quantity</font></td>";
echo"<td><font color='black'>Balance &nbsp;Quantity</font></td>";
echo"<td><font color='black'>Store</font></td>";
echo"</tr>";

while($row = mysqli_fetch_array($result))
  {
                 echo '<tr width:100px;><form action="MonthlyReport.php " method="post" >';
				         echo "<td><input style='width:100px;' value='".$row[0]."' name=date readonly id= did> </td>";
					       echo "<td><input style='width:90px;' value='".$row[1]."' name=productid readonly id=eid> </td>";
                 ...
                 echo "<td><input style='width:180px;' value='".$row[4]."' name=producttype readonly id=eid> </td>";
                 echo "<td><input style='width:90px;' value='".$row[5]."' name=unit readonly id=eid> </td>";
					       echo "<td><input style='width:150px;' value='".$row[6]."' name=deliveredquantity readonly > </td>";
					       echo "<td><input style='width:120px;' value='".$row[7]."' name=balancequantity readonly > </td>";
					       echo "<td><input style='width:100px;' value='".$row[8]."' name=store readonly > </td>";
...
<td><font color='black' >Sign____________</font></td><br><br>
 ...
 <center>
  <button onclick="window.print();" name="print" id="print" height="100px" width="50px" class="print" buttonprint>print </button>
