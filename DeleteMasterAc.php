...
<form action="DeleteMasterAc.php" method="post" name="delete" onsubmit="return checkvalidation()">
...
include ('Conn.php');
     $select_query = "select * from masterlist where unit='pcs'"; 
         if (isset($_POST['DeleteMasterAc'])) {
	          $productid=$_POST['productid'];
		        $delete_query = "delete from masterlist where unit='pcs' and productid = '$productid'";
             if(mysqli_query($data,$delete_query)) {
	            echo '<p style="color:green">Delete Data success!</p>';
        }
		 }
    ...
    echo "<table id='qq' style='width:1000px;border:1px solid #336699;border-radius:10px;' align='center'><font color=white>";
     ...
     while($row = mysqli_fetch_row($select_result)) {
					echo '<tr><form action="DeleteMasterAc.php" method="post">';
					...
					echo "<td><input type=submit name='DeleteMasterAc' value='Delete' Onclick='return ConfirmDelete();'> </td></form></tr>";
	              }
