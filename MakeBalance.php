		...
		if($result=mysqli_query($query,$select)){
			...
			if($feitchquantity < $withdraw){
				...
				if(mysqli_query($query,$update)){
					...
					echo"<script>alert('Update Balance Successful!!!')</script>";
...
$select_query = "SELECT  * FROM balancep where generalname='profile'"; 
     ...
     while($row = mysqli_fetch_row($select_result)) {
					echo '<tr width:200px;><form action="MakeBalance.php" method="post" >';
					...
					echo "<td><input name='MakeBalance' type=submit value=update Onclick='return ConfirmUpdate();'> </td></form></tr>";
	              }
				echo "</table>"; 
