...
<form action="MakeBalanceAc.php" method="post" name="update" onsubmit="return checkvalidation()">
<fieldset id=""><legend align="center"> <font face='calibri' size='5px' color="brown">Make a Balance Page</font></legend> <br><br>
 <?php 
 include('Connection.php');

	if (isset($_POST['MakeBalanceAc'])) {
		$date=$_POST['date'];
		...
		if($result=mysqli_query($query,$select)){
			...
			if($feitchquantity < $withdraw){
				echo"<script>alert('Insufficient Balance !!!')</script>";
			}
			else{
				$quantityresss=$feitchquantity-$withdraw;
				...
				deliveredquantity='$withdrawres', balancequantity='$quantityresss', store='$store' where productname='$iname' and producttype='$itype'";
...
include ('Conn.php');
$select_query = "SELECT  * FROM balancep where generalname='Accesseries' order by producttype"; 
     ...
     while($row = mysqli_fetch_row($select_result)) {
					echo '<tr width:200px;><form action="MakeBalanceAc.php" method="post" >';
					...
					echo "<td><input name='MakeBalanceAc' type=submit value=update Onclick='return ConfirmUpdate();'> </td></form></tr>";
	              }
				echo "</table>"; 
